...
export const login = asyncHandler(async (req, resp) => {
  try {
    ...
    logger.info('DB query result', { rowCount: result.rows.length, userFound: !!result.rows[0] });

    const user = result.rows[0];

    // Generic error — don't reveal whether email exists
    if (!user || !user.is_active) {
      logger.warn('User not found or inactive', { email: email.toLowerCase().trim(), userExists: !!user, isActive: user?.is_active });
      return res.unauthorized(resp, 'Invalid email or password');
    }

    logger.info('User found, comparing password', { userId: user.id, email: user.email });
